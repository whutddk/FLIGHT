...
uint8 TX_ADDRESS[NRF24L01_TX_ADR_LEN]={0x34,0x43,0x10,0x10,0x01}; //发送地址
uint8 RX_ADDRESS[NRF24L01_RX_ADR_LEN]={0x34,0x43,0x10,0x10,0x01}; //接收地址(本机地址,仅用于接收模式)
//NRF24L01复位之后用于检测
...
uint8 LPLD_Nrf24L01_debug(void)
{
  uint8 Temp;
  Temp=LPLD_Nrf24L01_ReadBuf(NRF24L01_READ_REG + NRF24L01_RF_CH, &debug_buf,1);
  asm("nop");
  Temp=LPLD_Nrf24L01_ReadBuf(NRF24L01_READ_REG + NRF24L01_STATUS, &debug_buf,1);
  asm("nop");
  Temp=LPLD_Nrf24L01_ReadBuf(NRF24L01_READ_REG + NRF24L01_EN_RXADDR, &debug_buf,1);
  asm("nop");   
  return Temp;
}
...
uint8 LPLD_Nrf24L01_Init(void)
{
  ...
  spi_init_param.SPI_ModeSelect = SPI_MODE_MASTER;
  ...
  ce_io_param.GPIO_PinControl = IRQC_DIS ;
...
uint8 LPLD_Nrf24L01_TxModeInit(void)
{
  ...
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_SETUP_RETR, 0x00);  //设置自动重发时间和次数：500us + 86us, 10 retrans...
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_RX_PW_P0, NRF24L01_PLOAD_LEN);
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_RF_CH, 0);         //设置信道工作为2.4GHZ，收发必须一致
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_RF_SETUP, 0x07);    //设置发射速率为1MHZ，发射功率为最大值0dB
...
{ 
  ...
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_EN_AA, 0x01);      //频道0自动,ACK应答允许      
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_EN_RXADDR, 0x01);  //允许接收地址只有频道0 
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_SETUP_RETR, 0x00); // 500us + 86us, 10 retrans... 	
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_RF_CH, 0);        //设置信道工作为2.4GHZ，收发必须一致  
  Temp=LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG + NRF24L01_RF_SETUP, 0x07);   //设置发射速率为1MHZ，发射功率为最大值0dB
