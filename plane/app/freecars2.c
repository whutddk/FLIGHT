...
void UartDebug()
{
  ...
  if( (uint32)UartData[7] == 1000 && (uint32)UartData[8] == 1000 )
  {
    flag_dump = 0;//双1000，飞机上电
    //LPLD_GPIO_Output_b(PTD, 10, 1);//飞机上电
    PWM0_init();//飞控启动自锁，占空比2000 
  }
  if( (uint32)UartData[7] == 2000 && (uint32)UartData[8] == 2000 )
  {
    flag_lock = 0;//双2000，飞控解锁
    safty_lock();
    
  }
  if( (uint32)UartData[7] == 5000 && (uint32)UartData[8] == 5000 )
  {
    MCU_Reset();
  }
...
//                ctrl.pitch.kp = UartData[1];
//                //ctrl.pitch.aim = UartData[2];
//                ctrl.pitch.kd = UartData[3];
//                ctrl.roll.kp = UartData[4];
//                //ctrl.roll.aim = UartData[5];
//                ctrl.roll.kd = UartData[6];
//    
//    ctrl.yaw.kp = UartData[1];
//    //    ctrl.yaw.ki = UartData[2];
//    ctrl.yaw.kd = UartData[3];
