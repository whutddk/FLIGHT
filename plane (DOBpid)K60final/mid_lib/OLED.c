...
void OLEDIO_init()
{
GPIO_InitTypeDef OLEDIO_inittype;
  OLEDIO_inittype.GPIO_PTx = PTA;
OLEDIO_inittype.GPIO_Pins = GPIO_Pin6 | GPIO_Pin7;
  OLEDIO_inittype.GPIO_PinControl = INPUT_PULL_DIS;
OLEDIO_inittype.GPIO_Dir = DIR_OUTPUT;
  OLEDIO_inittype.GPIO_Output = OUTPUT_H;

  LPLD_GPIO_Init(OLEDIO_inittype);
  
  OLEDIO_inittype.GPIO_PTx = PTE;
  OLEDIO_inittype.GPIO_Pins = GPIO_Pin26 | GPIO_Pin27 | GPIO_Pin28;
  OLEDIO_inittype.GPIO_PinControl = INPUT_PULL_DIS;
  OLEDIO_inittype.GPIO_Dir = DIR_OUTPUT;
  OLEDIO_inittype.GPIO_Output = OUTPUT_H;
...
void OLED_WrDat(uint8 data)
{
  ...
  LPLD_GPIO_Output_b(PTA,7,0);
  //asm("nop");
  while(i--)
  {
    if(data&0x80){LPLD_GPIO_Output_b(PTA,6,1);}
    else{LPLD_GPIO_Output_b(PTA,6,0);}
    LPLD_GPIO_Output_b(PTA,7,1);
    asm("nop");;
    //asm("nop");
    LPLD_GPIO_Output_b(PTA,7,0);;
    data<<=1;
...
void OLED_WrCmd(uint8 cmd)
{
  ...
  while(i--)
  {
    if(cmd&0x80){LPLD_GPIO_Output_b(PTA,6,1);}
    else{LPLD_GPIO_Output_b(PTA,6,0);;}
    LPLD_GPIO_Output_b(PTA,7,1);
    asm("nop");
    //asm("nop");
...
void OLED_Init(void)
{
  ...
  LPLD_GPIO_Output_b(PTE,26,0); ///使能信号端，拉低时正常使用
  LPLD_GPIO_Output_b(PTE,28,0);
  OLED_DLY_ms(50);
  LPLD_GPIO_Output_b(PTE,28,1);
...
void OLED_P8x16Char(uint8 x,uint8 y,uint8 ch)
{
  uint8 c=0,i=0,j=0;
...
void OLED_PrintFloat(uint8 ucIdxX, uint8 ucIdxY, float64 sData)
{
  ...
  if(usData < 100000.0 && usData > -100000.0)
  {
    l  = (uint16)(usData / 10000) % 10;
    m  = (uint16)(usData / 1000) % 10;
    i  = (uint16)(usData / 100) % 10;
    j  = (uint16)(usData / 10) % 10;
    k  = (uint16) usData % 10;
    f1 = (uint16)(usData * 10) % 10;
    f2 = (uint16)(usData * 100) % 10;
    f3 = (uint16)(usData * 1000) % 10;
    f4 = (uint32)(usData * 10000) % 10;
    f5 = (uint32)(usData * 100000) % 10;
    f6 = (uint32)(usData * 1000000) % 10;
    
    
    if ((uint8)l != 0)
    {
      OLED_P8x16Char(ucIdxX+8,  ucIdxY, (uint8)l+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+8,  ucIdxY, " ");
    }
    
    if ( (uint8)m != 0 || (uint8)l != 0)
    {
      OLED_P8x16Char(ucIdxX+16, ucIdxY, (uint8)m+48);
    }
    else
    {
      OLED_P6x8Str(ucIdxX+16,  ucIdxY, " ");
    }
    if ( (uint8)m != 0 || (uint8)l != 0 || (uint8)i != 0)
    {
      OLED_P8x16Char(ucIdxX+24, ucIdxY, (uint8)i+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+24,  ucIdxY, " ");
    }
    if ( (uint8)m != 0 || (uint8)l != 0 || (uint8)i != 0 || (uint8)j != 0)
    {
      OLED_P8x16Char(ucIdxX+32, ucIdxY, (uint8)j+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX +32,  ucIdxY, " ");
    }
    
    OLED_P8x16Char(ucIdxX+40, ucIdxY, (uint8)k+48);
    
    
    OLED_P8x16Char(ucIdxX+48, ucIdxY, (uint8) '.' );
    if ( (uint8)f6 != 0 || (uint8)f5 != 0 || (uint8)f4 != 0 || (uint8)f3 != 0 || (uint8)f2 != 0 || (uint8)f1 != 0 )
    {
      OLED_P8x16Char(ucIdxX+56, ucIdxY, (uint8)f1+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+56,  ucIdxY, " ");
    }
    if ( (uint8)f6 != 0 || (uint8)f5 != 0 || (uint8)f4 != 0 || (uint8)f3 != 0 || (uint8)f2 != 0)
    {
      OLED_P8x16Char(ucIdxX+64, ucIdxY, (uint8)f2+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+64,  ucIdxY, " ");
    }
    if ( (uint8)f6 != 0 || (uint8)f5 != 0 || (uint8)f4 != 0 || (uint8)f3 != 0)
    {
      OLED_P8x16Char(ucIdxX+72, ucIdxY, (uint8)f3+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+72,  ucIdxY, " ");
    }
    if ( (uint8)f6 != 0 || (uint8)f5 != 0 || (uint8)f4 != 0)
    {
      OLED_P8x16Char(ucIdxX+80, ucIdxY, (uint8)f4+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+80,  ucIdxY, " ");
    }
    if ( (uint8)f6 != 0 || (uint8)f5 != 0 )
    {
      OLED_P8x16Char(ucIdxX+88, ucIdxY, (uint8)f5+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+88,  ucIdxY, " ");
    }
    if ( (uint8)f6 != 0 )
    {
      OLED_P8x16Char(ucIdxX+56, ucIdxY + 2, (uint8)f6+48);
    }
    else
    {
      OLED_P8x16Str(ucIdxX+56,  ucIdxY + 2, " ");
    }
  }
  else
  {
    OLED_P8x16Str( ucIdxX, ucIdxY,"Error!");
  }
  return;
}
//==============================================================
