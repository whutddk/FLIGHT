...
void UartDebug()
{
  ...
  if(  (uint32)UartData[8] == 1000 )
  {
    flag_dump = 0;//双1000，飞机上电
    //LPLD_GPIO_Output_b(PTD, 10, 1);//飞机上电
    PWM0_init();//飞控启动自锁，占空比2000 
  }
  if( (uint32)UartData[8] == 2000 )
  {
    flag_lock = 0;//双2000，飞控解锁
    safty_lock();
    
  }
  if( (uint32)UartData[8] == 5000 )
  {
    MCU_Reset();
  }
  
  
  if ( !flag_lock && !flag_dump )
  {
    throttle = (uint32)UartData[0];
    
    ctrl.pitch.shell.kp = ctrl.roll.shell.kp = UartData[1];
    ctrl.pitch.shell.ki = ctrl.roll.shell.ki =UartData[2];
    ctrl.pitch.shell.kd = ctrl.roll.shell.kd =UartData[3];
    
    ctrl.pitch.shell.aim = ctrl.roll.shell.aim = UartData[4];
    
    ctrl.pitch.core.kp = ctrl.roll.core.kp = UartData[5];
    ctrl.pitch.core.ki = ctrl.roll.core.ki = UartData[6];
    ctrl.pitch.core.kd = ctrl.roll.core.kd = UartData[7];
      
//      ctrl.yaw.core.kp = UartData[1];
//      
//      ctrl.high.aim = UartData[2];
//        ctrl.high.kp = UartData[3];
//  ctrl.high.ki = UartData[4];
//  ctrl.high.kd = UartData[5];
  }
...
void UartCmd(uint8 cmdnum,uint8 cmddata)///
{
  switch(cmddata)
  {
    ...
    case(11):break;//F11
    case(12):break;//F12
    case(100):flag_lock = 1;break;//PAUSE//飞控锁定
    case(101):quick_land();break;//HOME
    case(102):break;//PG UP
    case(103):break;//PF DN
    case(104):flag_dump = 1;break;//END//掉电
  }
}
